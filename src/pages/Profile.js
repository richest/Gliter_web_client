...
import { addDefaultSrc, checkLoginRole, fetchPreviousPage, returnDefaultImage } from "../commonFunctions";
...
const location = useLocation()
  ...
  const [randomCallsChecked, setRandomCallChecked] = useState(false)
  ...
  const shareUrl = 'https://clickmystar.in/';
  ...
  const ProfileData = async (interests_list) => {
    ...
    let randomCalls = !!data.call_status ? true : false
    setRandomCallChecked(randomCalls)
    dispatch(
      profile({
        profile: data
      })
  ...
  const handleBuyCoins = () => {
    ...
    axios.post(GET_ALL_COIN_PACKAGE, bodyParameters)
      .then((response) => {
...
const changeRandomCalls = (e) => {
  const bodyParameters = {
    session_id: sessionId,
  }
  axios.post(CHANGE_CALL_STATUS_API, bodyParameters)
    .then((response) => {

      if (response.data.status == 200) {
        if (response.data.call_status == 1) {
          setRandomCallChecked(true)
        } else {
          setRandomCallChecked(false)
        }
      }
    }, (error) => {
      if (error.toString().match("403")) {
          ...
          <div className="row">
            <div className="col-md-4 border-rt full-width">
              ...
              <div className="user-profile__options becomevip-wrapper__innerblock">
                ...
                {
                    !pageloading &&
                  <li><a href="javascript:void(0)" id="edit-profile" onClick={handleShowGallery}><img src="/assets/images/gallery.png" style={{width: "25px"}} alt="Edit Profile" />
              ...
              <div className="user-profile__options becomevip-wrapper__innerblock">
                <ul>
                  <li><a href="javascript:void(0)" id="blacklist" onClick={handleBlockList}>
                    <h6 className="mb-0"><img src="/assets/images/blacklist-icon.png" alt="Blacklist" />Blocklist</h6> <i className="fas fa-chevron-right" />
                  </a></li>
          ...
          <div className="user-profile__options becomevip-wrapper__innerblock custom-checkbox">
            <ul>
              ...
              <li>
                <h6>Random Calls</h6>
                <div className="custom-checkbox__status">
                  <span className="checkbox-state">{!!randomCallsChecked ? "Active" : "Inactive"}</span>
                  <label className="switch">
                    <input type="checkbox" checked={!!randomCallsChecked ? "checked" : ""} onChange={(e) => changeRandomCalls(e)} />
                    <span className="slider" />
                  </label>
                </div>
              </li>
          ...
          <div className="membership-plans">
            {coinPackage.map((item, i) => (
              <div className="membership-plans__block  active mt-2">
                <a href="javascript:void(0)" className={item.is_buy ? "justify-content-start disable-coin-package" : "justify-content-start"} onClick={(e) => StripeCoinHandler(item.id)}>
                  ...
                  <div className="buy-gifts_price text-white">
                    {
                      item.is_buy &&  <span className="mb-0">You can buy this only one time</span>
                    }

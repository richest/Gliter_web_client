...
import { ADD_STATUS, FRIENDLIST_API, GET_STATUS, LIKE_STATUS_API, DETUCT_THOUSAND_COIN, VIEW_STATUS_API, PAID_MEDIA_CHAT, PURCHASE_STORY } from '../components/Api';
...
import { addDefaultSrc, checkLoginRole, fetchPreviousPage, openNewWindow, returnDefaultImage, useForceUpdate } from "../commonFunctions";
import useToggle, { removeDublicateFrds, restrictBack } from '../components/CommonFunction';
...
const [storiesItem , setStoriesItem] = useState({})
  ...
  const customCollapsedComponent = ({coins, totalviews, status_id, total_likes, is_liked, paid_status }) => {
    return <>
      <div className="status_footer">
        <span className="status_view"><img src="/assets/images/eye-icon.png" alt="eye" />{totalviews}</span>
        <div className="status_like" style={{ cursor: paid_status ? "pointer" : "default" }} onClick={() => likeStatus(status_id, is_liked, paid_status)}>
          <span ><img src="/assets/images/heart-icon.png" alt="like status" /> {TLikesStatus}</span>
        </div>
        {!paid_status?<span className="status_purchase" style={{cursor: "pointer"}} onClick={() => purchaseStatus(coins ,status_id)}>purchase</span>:""}
      </div>
    </>
  }

  //purchase status
  const purchaseStatus = async (coin ,statusId) => {
    // hit api to deduct the coins
    const bodyParameters = { session_id: localStorage.getItem("session_id"), coins: coin, frd_id: statusData.user_id ,status_id:statusId }
    const { data: {data, message, status_code, error } } = await axios.post(PURCHASE_STORY, bodyParameters)
 
    if (status_code == 200) {
   for(let i in storyData){
     if(storyData[i].coins ==coin){
      storyData[i].paid_status= true
  ...
  if (!!storyData) {
    for (let i in storyData) {
      if (storyData[i].paid_status == false) {
        if (storyData[i].coins == 200) { storyData[i].url = "/assets/images/Coins_200.jpg" }
        else if (storyData[i].coins == 500) { storyData[i].url = "/assets/images/Coins_500.jpg" }
        else if (storyData[i].coins == 2000) { storyData[i].url = "/assets/images/Coins_2000.jpg" }
        else if (storyData[i].coins == 5000) { storyData[i].url = "/assets/images/Coins_5000.jpg" }
        else if (storyData[i].coins == 50) { storyData[i].url = "/assets/images/Coins_50.jpg" }
        else if (storyData[i].coins == 100) { storyData[i].url = "/assets/images/Coins_100.jpg" }
...
console.log(storyData , "storyData...")
  ...
  useEffect(() => {
    ...
    const checkBlockStatus = (streamerDetails, blockedLists) => {
      const user_id_storage = !!localStorage.getItem("user_id") ? Number(localStorage.getItem("user_id")) : "";
      const live_streamer_id = streamerDetails.user_id;
      const blocked_users = blockedLists.blocked_users;
      const blocked_countries = blockedLists.blocked_countries;
      let streamer_blocked_countries = [];
      let is_blocked = false;
      const client_country_code = !!userData ? userData.country_code : "";
  
      blocked_users.forEach((users, index) => {
        if ((users.user_id == user_id_storage && users.blockedUserID == live_streamer_id) ||
          (users.blockedUserID == user_id_storage && users.user_id == live_streamer_id)) {
          is_blocked = true;
          return;
        }
      })
      if (!is_blocked) {
        for (let i in blocked_countries) {
          if (blocked_countries[i].user_id == live_streamer_id) {
            streamer_blocked_countries.push(blocked_countries[i].country_code)
          }
        }
        streamer_blocked_countries.forEach((countries, index) => {
          if (countries == client_country_code) {
            is_blocked = true
          }
        })
      }
    ...
    SOCKET.off('live_friends').on('live_friends', (data, user, blockedLists) => {
      if (!!userData && user.user_id == userData.user_id) {
        console.log(data, blockedLists, "changoooooooooooooooooooooooooooooooooo")
        ...
        for (let i in new_live_frds_list) {
          if (new_live_frds_list[i].id == 251) {
            // alert(JSON.stringify(new_live_frds_list[i]))
          }
          const is_blocked = checkBlockStatus(new_live_frds_list[i], blockedLists);
          if (!is_blocked) {
            if (new_live_frds_list[i].is_online && new_live_frds_list[i].is_live) {
              if (new_live_frds_list[i].user_id == 18) {
                console.log(new_live_frds_list[i], "sldkjgnvkdvhvf", countryCode, userData.country_code)
              }
              ...
              for (let f in frd_blocked_countries) {
                if (countryCode == (frd_blocked_countries[f])) {
            ...
            new_live_frds_list.splice(i, 1)
          }
        }

        changeLiveFriendList = new_live_frds_list;
        for (let j in new_live_frds_list) {
          if (!new_live_frds_list[j].firstName && !new_live_frds_list[j].lastName && !new_live_frds_list[j].image) {
            console.log(new_live_frds_list[j].user_id, "wrongggg.....................")
          }
        }
    ...
    SOCKET.off('start_your_live_video_now').on('start_your_live_video_now', (data) => {
      if ((data.user_id == userData.user_id) && data.channel_id && data.channel_name) {
        ...
        localStorage.setItem("prevPage", location.pathname)
        const page = '/' + data.user_id + '/' + data.channel_id + '/' + data.channel_name + '/' + localStorage.getItem("socket_id") + '/live-video-chat'
        // openNewWindow(page);
        window.location = page
      }
    });
                      ...
                      {liveFriendList.map((item, i) => (
                          <div className="users-listing__slider__items" id={item.user_id}>
                            <div className="users-listing__slider__items__image" id="modal" data-toggle="modal" onClick={() => setFriendId(item.user_id)}>
                              {!!liveFriendList ? <img onError={(e) => addDefaultSrc(e)} src={!!item.image ? "https://clickmystar.in/glitterclone/glitter-101/public/profile_images/" + item.image : returnDefaultImage()} alt="marlene" /> : ""}
       ...
       return <div className="status-bar__items">
            {items.statuses_type==1 ?
             <img onError={(e) => addDefaultSrc(e)} src={!!items.file ? items.file : returnDefaultImage()} alt="status" /> :
              ...
              convertToHtml(`<p>${items.file.replace("http://167.172.209.57/glitterclone/glitter-101/public/profile_images/", "")}</p>`))
             } </div>
        })}
        </OwlCarousel>
      </div>
                ...
                <>
                  ...
                  <div className="image-coins d-flex">
                    ...
                    <div className="coin-price">
                      ...
                      <label for="coin-value2">200 coins</label>
                    ...
                    <div className="coin-price">
                      ...
                      <label for="coin-value3">500 coins</label>
                    ...
                    <div className="coin-price">
                      ...
                      <label for="coin-value4">2000 coins</label>
                    ...
                    <div className="coin-price">
                      ...
                      <label for="coin-value5">5000 coins</label>

...
import { selectUser, userProfile, videoCall, audioCall, randomMessageMe } from "../features/userSlice";
import { NotificationManager } from 'react-notifications';
import useToggle, { detectMob, isMobile, removeDublicateFrds, restrictBack } from '../components/CommonFunction';
import { useHistory,useLocation } from "react-router-dom";
import { addDefaultSrc, checkLoginRole, fetchPreviousPage, getCurrentDateTime, openNewWindow, returnDefaultImage, useForceUpdate } from "../commonFunctions";
...
let checkLastFrdsMsgInterval, my_friends_list = [], myFriendUserId = "",
    medias_temp = [], new_page = 1, scrollPage = 1;
...
const ChatBox = (props) => {
    ...
    const purchaseMedia = async () => {
        ...
        const bodyParameters = { session_id: localStorage.getItem("session_id"), coins: purchaseItem.coins, frd_id: FriendUserId }
    ...
    const componentWillUnmount = () => {
        ...
        //----------new addeed ----------
        SOCKET.emit('message_read_unread_status', { "session_id": sessionId, "receiver_id": FriendUserId, is_read: 0, 'sender_id': !!userData ? userData.user_id : "" });
        ...
        if (data.coins > 0) {
            ...
            setImageFull((data.media.match("gifts_icons")) ? { open: false, media: null } : {
                open: true, media: (!!data.media ?
                    (
                        data.coins == 50 ?
                            "/assets/images/Coins_50.jpg"
                            :
                            (
                                data.coins == 100 ?
                                    "/assets/images/Coins_100.jpg"
                                    :
                                    (
                                        data.coins == 200 ?
                                            "/assets/images/Coins_200.jpg"
                                            :
                                            (
                                                data.coins == 500 ?
                                                    "/assets/images/Coins_500.jpg"
                                                    :
                                                    (
                                                        data.coins == 2000 ?
                                                            "/assets/images/Coins_2000.jpg"
                                                            :
                                                            (
                                                                data.coins == 5000 ?
                                                                    "/assets/images/Coins_5000.jpg"
                                                                    :
    ...
    const checkPaidMedia = (data) => {
        ...
        switch (data.coins) {
            case 0:
                media = data.media
            case 50:
                media = "/assets/images/Coins_50.jpg"
            case 100:
                media = "/assets/images/Coins_100.jpg"
            case 250:
                media = "/assets/images/Coins_250.png"
            case 500:
                media = "/assets/images/Coins_500.jpg"
    ...
    useEffect(() => {
        ...
        SOCKET.off('check_receiver_message_status').on('check_receiver_message_status', function (list) {
            if (list.friend_id == FriendUserId) {
            ...
            if (!!messages) {
                if ((messages.obj.user_from_id == userData.user_id && messages.obj.user_to_id == receiver_id)
                    ...
                    if (!!messages.obj.warningMessage) {
                        ...
                        SOCKET.emit('ping_frd_my_unread_message', { "user_id": FriendUserId, toMe: false });
                        SOCKET.emit('get_unread_frd_messages', {
                            user_id: FriendUserId,
                            check_user: FriendUserId,
                            toMe: false
                        });
                        setWarningMessage('');
                ...
                <div className="container">
                    <div className="row panel messages-panel">
                                        ...
                                        <Scrollbars onScrollFrame={handleScroolFriend} style={{ height: 400 }} autoHide >
                                            ...
                                            {FriendList.map((item, i) => {
                                                return <li className="nav-item w-100">
                                                    <a className="nav-link" href="#chat-field" data-toggle="tab" data-id={item.user_id} role="tab" onClick={() => {setFriendId(item.user_id); myFriendUserId = item.user_id ; setMessagePage(1); scrollPage = 1; setPageScrool(true) ;setAudioTime(null);clearInterval(intervalss);setDummyMediaRc(null);}}>
                                    ...
                                    <div className="message-top d-flex flex-wrap align-items-center justify-content-between">
                                        {
                                            <div className="chat-header-info d-flex align-items-center">
                                                {!!AllData &&  !loading ? <img alt="Mia" style={{ cursor: "pointer" }} className="img-circle medium-image" onClick={() => history.push(`/${FriendUserId}/single-profile`)} src={AllData.profile_images[0].replace("/glitterclone/", "/")} /> : ""}
                                                                    ...
                                                                    <div className={!!data.media && data.media.match(".mp4") ? "message-body video-added" : (!!data.audio ? "message-body audio-added" : "message-body")}>
                                                                        {
                                                                            ...
                                                                            <div className="media-socket">
                                                                                {(data.media.match(".mp4") && data.coins == 0) ?
                                                                                    <>
                                                                                        <video onPlay={(e) => handlePlay(e)} id={"videoPlayer" + data.message_id} src={"https://clickmystar.in:444/glitter_node/" + data.media}
                        ...
                        <Modal className="fullSize-image-modal text-center" show={purchaseItem.purchase} onHide={() => showPurchaseItem({ purchase: false, message_id: "", coins: 0 })} backdrop="static" keyboard={false}>
                            ...
                            <a href="javascript:void(0)" className="modal-close" onClick={() => showPurchaseItem({ purchase: false, message_id: "", coins: 0 })}><img src="/assets/images/btn_close.png" /></a>
                        </Modal>
                        ...
                        <Modal className="theme-modal" id="upload-media-modal" show={openVideo} onHide={() => setOpenVideo(false)} backdrop="static" keyboard={false}>
                            ...
                            <a href="javascript:void(0)" onClick={closeUploadModel} className="modal-close" ><img src="/assets/images/btn_close.png" /></a>
                        </Modal>

...
import {changeImageLinkDomain, checkLiveDomain, removeVideoListRightClick, returnDefaultImage} from "../commonFunctions";
import NotificationManager from "react-notifications/lib/NotificationManager";
import { STREAMER_REPORT_API } from "../components/Api";
import { restrictBack } from "../components/CommonFunction";

var totalSeconds = 0, timeCounterLive, timerVar;

let videoCallStatus = 0, videoCallParams, interval, connectedPeopleInterval, connectedPeopleIntervalBeforeAck, updateConnectedPeopleTimeInterval,
...
const AudioChat = () =>{
  ...
  const componentWillUnmount = (reactLeave) => {
    ...
    clearInterval(connectedPeopleIntervalBeforeAck);
  ...
  useEffect(() => {
    console.log(time, "lkfdhgkdjfhb")
    // update table(audio call) with time.... 
  }, [time])
  ...
  // }
    ...
    if (!params.channel_name) {
      ...
      document.getElementsByClassName("vc-screen")[0]
          ...
          // window.setTimeout(() => {
          console.log(localStorage.getItem("videoCallPageRefresh"), "checkkkkkkkkkkkkkkkkkkkkkkkkkk")
      if (!localStorage.getItem("videoCallPageRefresh")) {
        connectedPeopleIntervalBeforeAck = window.setInterval(() => {
          SOCKET.emit("get_connected_people_video_call_before_acknowledge", {
            sender: {user_from_id: videoCallParams.user_from_id, session_id: localStorage.getItem("session_id")},
              reciever_id: videoCallParams.user_to_id,
              channel_name: videoCallParams.channel_name,
              type: 1,
    ...
    SOCKET.off('sender_show_video_call').on('sender_show_video_call', (data) => {
      ...
      if ((data.user_from_id == videoCallParams.user_from_id && data.user_to_id == videoCallParams.user_to_id)
        ...
        if (!hostCallCheck) {
          ...
          // check from table(audio_call) the connection with 1.....
          timerVar = setInterval(countTimer, 1000);
          console.log(time, "time----host")
        }
        else {
        ...
        if (!!userData && (data.user_to_id == userData.user_id)) {  // receiver
          timerVar = setInterval(countTimer, 1000);
          console.log(time, "time----audience")
  ...
  SOCKET.off('get_connected_people_video_call_before_acknowledge').on('get_connected_people_video_call_before_acknowledge', (data) => {
    if ((data.user_from_id == videoCallParams.user_from_id && data.user_to_id == videoCallParams.user_to_id)
        ||
        (data.user_from_id == videoCallParams.user_to_id && data.user_to_id == videoCallParams.user_from_id)
    ) { // check one-to-one data sync
      console.log((data.details)[0], "hgjhbfkguli")
      if ((data.details).length > 0) {
        if ((data.details)[0].call_status == 6) {
          const onlySender = true;
          const traffic = true;
          endCall(true, onlySender, traffic)
        }
      }
    }
  });
...
restrictBack()
  ...
  const audioManageCoinsTimeViews = (show_host_coins) => {
    console.log("is_streamer_calling", params, params)
  ...
  const endCall = (showMsg, onlySender, traffic) => {
            ...
            <div className="vc-action-tab ml-auto mr-4 position-relative">
              <div className="vc-action-btn" onClick={actionButton}>
               ...
               <ul className="action-menu" id="actionBtn">
                {/* <li>
                  {
                    (!!userData && !!videoCallParams && userData.user_id == videoCallParams.user_to_id) &&
